...
Uses aioodbc driver with SQLAlchemy.
"""
...
class SQLServerAdapter:
    """SQL Server database adapter using aioodbc driver."""
    ...
    async def init(self) -> None:
        ...
        if self._engine is not None:
            logger.warning("SQL Server engine already initialized")
            return

        database_url = settings.database_url
        logger.info(f"Initializing SQL Server connection to {settings.db_host}:{settings.db_port}/{settings.db_name}")
    ...
    async def close(self) -> None:
        ...
        if self._engine:
            logger.info("Closing SQL Server connections")
