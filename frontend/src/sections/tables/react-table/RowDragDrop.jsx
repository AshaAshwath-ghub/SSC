...
function ReactTable({ defaultColumns, defaultData }) {
  ...
  const handleDragEnd = (event) => {
    ...
    if (active && over && active.id !== over.id) {
      const draggedRowIndex = table.getRowModel().rows.findIndex((row) => `row-${row.id}` === String(active.id));
      const targetRowIndex = table.getRowModel().rows.findIndex((row) => `row-${row.id}` === String(over.id));
